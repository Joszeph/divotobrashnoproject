...
function PostSladki({post}:Props) {
       ...
       <NextSeo {...SEO} />
        <div className={styles.about}>
            <div className={styles.aboutEl}>  
                <div className={styles.backBtnRes}><Link href="/sladki"><a>назад</a></Link></div>
                ...
                <article className={styles.aboutText}>
                    ...
                    content = {post.body}/>
                    <div className={styles.backBtn}><Link href="/sladki"><a>назад</a></Link></div>
                </article>
                </div>
            </div>
        </div>
...
export const getStaticPaths = async () => {
    ...
    const paths = posts.map((post: Sladki)=>({
        params:{
...
export const getStaticProps: GetStaticProps = async ({params}) => {
    const query = `*[_type == "sladki" && slug.current == $slug][0]{
        _id,
        createdAt,
        title,
        description,

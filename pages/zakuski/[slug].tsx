...
function PostZakuski({post}:Props) {
   ...
   <main className={styles.main}>
        ...
        <div className={styles.about}>
            <div className={styles.aboutEl}>  
                <div className={styles.backBtnRes}><Link href="/zakuski"><a>назад</a></Link></div>
                ...
                <article className={styles.aboutText}>
                    ...
                    content = {post.body}/>
                    <div className={styles.backBtn}><Link href="/zakuski"><a>назад</a></Link></div>
                </article>
                </div>
            </div>
        </div>
...
export const getStaticPaths = async () => {
    ...
    const paths = posts.map((post: Zakuski)=>({
        params:{
...
export const getStaticProps: GetStaticProps = async ({params}) => {
    const query = `*[_type == "zakuski" && slug.current == $slug][0]{
        _id,
        createdAt,
        title,
        description,
